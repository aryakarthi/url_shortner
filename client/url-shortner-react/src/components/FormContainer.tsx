...
const FormContainer: React.FunctionComponent<IFormContainerProps> = (props) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.post(`${serverUrl}/shortUrl/new`, {
        fullUrl: fullUrl,
      });
      setFullUrl("");
      updateReloadState();
    ...
    <div className="container mx-auto p-2">
      <div className="bg-banner my-8 rounded-xl bg-cover bg-center">
        <div className="w-full h-full rounded-xl p-20 backdrop-brightness-50">
          <h2 className="text-white text-4xl text-center pb-4">URL Shorty</h2>
          ...
          <p className="text-white text-center pb-4 text-sm font-thin">
            free tool to shorten a URL or reduce link, Use our URL Shorty to
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex">
              <div className="relative w-full">
                ...
                <button
                  type="submit"
                  className="absolute top-0 end-0 p-2.5 text-sm font-medium h-full text-white bg-blue-700 rounded-r-lg border border-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300"
                >
                  Shorten URL
                </button>
              </div>
